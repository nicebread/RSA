#' @export
demoSRR <- function(a4=0, C=0, S=0, bm=0, type="3d", zlim=c(-2, 2), xlim=c(-2, 2), ylim=c(-2, 2), xlab="X", ylab="Y", zlab="Z", points = TRUE, model="full", project=c("PA1", "PA2"), legend=FALSE, coefs=TRUE, ...) {
	...
	setAllBlack <- function() {
		sapply(list(I_a4, I_C, I_S), tkconfigure, foreground="black")
	}
